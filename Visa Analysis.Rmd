...
```{r}
ggplot(data = VisaData, mapping = aes(x = (WORK_STATE = "West Virginia"), fill = JOB_TITLE_SUBGROUP)) + scale_x_discrete(guide=guide_axis(n.dodge=10)) +
  geom_bar()

ggplot(data = VisaData, mapping = aes(x = (WORK_STATE = "Washington"), fill = JOB_TITLE_SUBGROUP)) + scale_x_discrete(guide=guide_axis(n.dodge=10)) +
  geom_bar()

ggplot(data = VisaData, mapping = aes(x = (WORK_STATE = "New York"), fill = JOB_TITLE_SUBGROUP)) + scale_x_discrete(guide=guide_axis(n.dodge=10)) +
  geom_bar()

```


The states with the 4 highest average salaries for Visa Applicants are West Virgina, California, Washington, and New York. California, Washington, and New York are all huge tech hubs with Silicon Valley, Seattle, and Manhatten all within their states. 


```{r}

ggplot(data = VisaData, mapping = aes(x = (WORK_STATE = "New Mexico"), fill = JOB_TITLE_SUBGROUP)) + scale_x_discrete(guide=guide_axis(n.dodge=10)) +
  geom_bar()
